...
const ImageUploadModal = ({
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const res = await fetch(
        `http://${process.env.HOST}:3000/api/users/${currentUser._id}?action=upload`,
        {
          method: "PUT",
          mode: "cors",
          credentials: "include",
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const res = await fetch(`http://${process.env.HOST}:3000/${imageUrl}`, {
          method: "GET",
          mode: "cors",
          credentials: "include",
        });
  ...
  return (
    <div
      className={`imageUploadModal ${isOpen ? `imageUploadModal--open` : ""}`}
    >
      <Formik
      ...
      >
        {({
          ...
          <Form
            className="imageUploadModal__form"
            autoComplete="off"
            noValidate
          >
            <header className="imageUploadModal__header">
              <h1 className="imageUploadModal__title">
                Upload {capitalizeFirstLetter(type)}
              </h1>
              <FontAwesomeIcon
            ...
            <div className="imageUploadModal__thumbnailWrapper">
              {initialThumbnailUrl ? (
                <img
                  className={`imageUploadModal__thumbnail ${
                    type === "avatar"
                ...
                <Skeleton
                  height={128}
                  containerClassName="imageUploadModal__skeleton"
                />
              )}
              <ErrorMessage
                className="imageUploadModal__feedbackError"
                name={type}
                component="div"
              />
            </div>
            <div className="imageUploadModal__buttons">
              <input
                ...
                accept="image/*"
                onChange={(e) =>
                  handleFileChange(e, type, setFieldValue, setFieldTouched)
                }
                ref={fileInputRef}
              />
              <button
              ...
              >
                <FontAwesomeIcon
                  className="imageUploadModal__uploadIcon"
                  icon={faImage}
                />
                UPLOAD
              </button>
              <button
                className="imageUploadModal__button imageUploadModal__confirm"
                type="submit"
                disabled={isSubmitting || !dirty}
              >
                <FontAwesomeIcon
                  className="imageUploadModal__confirmIcon"
                  icon={faCircleCheck}
                />
                CONFIRM
              </button>
