...
const getModifiedValues = (values, initialValues) => {
  return Object.entries(values).reduce((prev, current) => {
    ...
    return value !== initialValues[key] ? { ...prev, [key]: value } : prev;
  }, {});
};
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const res = await fetch(
        `http://${process.env.HOST}:3000/api/users/${currentUser._id}?action=edit`,
        {
          method: "PUT",
          mode: "cors",
          credentials: "include",
            ...
            <div className="editInfoModal__formBody">
              <div className="editInfoModal__namesWrapper">
                <div className="editInfoModal__formGroup editInfoModal__formGroup--halfSize">
                  <label
                    className="editInfoModal__label"
                    htmlFor="editInfoModal__firstName"
                  >
                    First Name
                  </label>
                  <Field
                    className={`editInfoModal__input ${
                  ...
                  />
                  <ErrorMessage
                    className="editInfoModal__feedbackError"
                    name="firstName"
                    component="div"
                  />
                </div>
                <div className="editInfoModal__formGroup editInfoModal__formGroup--halfSize">
                  <label
                    className="editInfoModal__label"
                    htmlFor="editInfoModal__lastName"
                  >
                    Last Name
                  </label>
                  <Field
              ...
              <div className="editInfoModal__formGroup">
                <label
                  className="editInfoModal__label"
                  htmlFor="editInfoModal__bio"
                >
                  Bio
                </label>
                <Field
                  className={`editInfoModal__bio ${
                    touched.bio && errors.bio ? "editInfoModal__fieldError" : ""
                  }`}
                  as="textarea"
                  name="bio"
              ...
              <div className="editInfoModal__locationsWrapper">
                <div className="editInfoModal__formGroup editInfoModal__formGroup--halfSize">
                  <label
                    className="editInfoModal__label"
                    htmlFor="editInfoModal__location"
                  >
                    Location (City)
                  </label>
                  <Field
                ...
                <div className="editInfoModal__formGroup editInfoModal__formGroup--halfSize">
                  <label
                    className="editInfoModal__label"
                    htmlFor="editInfoModal__hometown"
                  >
                    Hometown
                  </label>
                  <Field
              ...
              <div className="editInfoModal__formGroup">
                <label
                  className="editInfoModal__label"
                  htmlFor="editInfoModal__occupation"
                >
                  Occupation
                </label>
                <Field
              ...
              <button
                className="editInfoModal__submit"
                type="submit"
                disabled={isSubmitting || !dirty}
              >
                <FontAwesomeIcon
                  className="editInfoModal__confirmIcon"
                  icon={faCircleCheck}
                />
                CONFIRM
              </button>
