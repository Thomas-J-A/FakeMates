    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const resUserData = await fetch(
          `http://${process.env.HOST}:3000/api/users/${userId}`,
          {
            method: "GET",
            mode: "cors",
            credentials: "include",
        ...
        if (
          !currentUser.friends.some((f) => f._id === bodyUserData._id) &&
          bodyUserData.isPrivate
        ) {
        ...
        const resPosts = await fetch(
          `http://${process.env.HOST}:3000/api/posts?userid=${userId}&page=1`,
          {
            method: "GET",
            mode: "cors",
            credentials: "include",
    ...
    const fetchPosts = async () => {
      ...
      try {
        const res = await fetch(
          `http://${process.env.HOST}:3000/api/posts?userid=${userId}&page=${currentPage}`,
          {
            method: "GET",
            mode: "cors",
            credentials: "include",
      ...
      <section className="profile__main">
        {error !== "userData" ? (
          <>
            ...
            <section className="posts">
              ...
              {posts.length > 0 && !hasMore && (
                <p className="posts__noMoreMessage">
            ...
            {userData && (
              <>
                <Backdrop
                  type="modal"
                  isVisible={Object.values(isOpenModal).some((v) => v)}
                  close={closeModal}
                />
                <ImageUploadModal
                  type="avatar"
          ...
          <div className="profile__error">
            <FontAwesomeIcon
              className="profile__errorIcon"
              icon={faTriangleExclamation}
            />
            <p className="profile__errorMessage">
              A little spot of bother fetching this info...
            </p>
          </div>
        )}
      </section>
