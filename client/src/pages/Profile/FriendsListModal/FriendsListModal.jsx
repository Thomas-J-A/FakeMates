...
const FriendsListModal = ({
  ...
  return (
    <div
      className={`friendsListModal ${isOpen ? "friendsListModal--open" : ""} ${
        isOwn ? "friendsListModal--own" : ""
      }`}
      ref={friendsListRef}
    >
      <header className="friendsListModal__header">
        <h1 className="friendsListModal__title">
          FakeMates{" "}
          <span className="friendsListModal__friendsCount">{`(${userData.friends.length})`}</span>
        </h1>
        <FontAwesomeIcon
